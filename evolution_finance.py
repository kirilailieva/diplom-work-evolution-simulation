# Повтаряме след reset: функцията simulate_scenario и ситуация 3
...
def simulate_scenario(a_matrix, name, x0, iterations=20):
    ...
    plt.title(f"Ситуация: Финансова криза – Развитие на стратегии")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
# ▶️ Симулация за Ситуация 3: Финансова криза / управление на ресурси
a_finance = np.array([
    [0.7, 0.3, 0.6, 0.5, 0.3],  # S1 – Адаптивност
    [0.2, 0.7, 0.2, 0.3, 0.3],  # S2 – Агресивност
    [0.6, 0.2, 0.9, 0.7, 0.3],  # S3 – Консервативност
    [0.5, 0.3, 0.7, 0.8, 0.4],  # S4 – Аналитичност
    [0.3, 0.3, 0.3, 0.4, 0.6],  # S5 – Интуитивност
])

simulate_scenario(a_matrix=a_finance, name="finance", x0=x_init)
